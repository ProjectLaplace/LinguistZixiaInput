...
final class UserDictionaryTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        userDictPath = NSTemporaryDirectory() + "laplace_test_user_dict_\(UUID().uuidString).db"
        let zhPath = Bundle.module.url(forResource: "zh_dict", withExtension: "db")!.path
        let jaPath = Bundle.module.url(forResource: "ja_dict", withExtension: "db")!.path
        engine = PinyinEngine(zhDictPath: zhPath, jaDictPath: jaPath, userDictPath: userDictPath)
    }

    override func tearDown() {
        try? FileManager.default.removeItem(atPath: userDictPath)
        super.tearDown()
    }
    ...
    private func type(_ text: String) -> EngineState {
        var state = EngineState.idle
        for char in text { state = engine.process(.letter(char)) }
        return state
    }

    private func space() -> EngineState { engine.process(.space) }

    func testLearnedPhraseAppearsAsCandidateOnRepeat() {
        // First input: "ziguangpinyin" composes per-syllable → "紫光拼音"
        type("ziguangpinyin")
        let firstCommit = space()
        XCTAssertNotNil(firstCommit.committedText, "Should commit something")

        // Second input: same pinyin should now include the learned phrase
        let state = type("ziguangpinyin")
        XCTAssertTrue(state.candidates.contains(firstCommit.committedText!),
            "Learned phrase should appear in candidates")
    }

    func testSingleCharNotSavedToUserDict() {
        // "shi" → 是 (single char), should not be saved
        type("shi")
        space()

        // Type again — candidates should be same as system dict, no user dict entry
        let state = type("shi")
        // 是 should be first (from system dict), and no duplicate
        XCTAssertEqual(state.candidates.first, "是")
        let count = state.candidates.filter { $0 == "是" }.count
        XCTAssertEqual(count, 1, "Single char should not be duplicated by user dict")
    }

    func testLearnedPhraseAppearsAfterSystemDictResults() {
        // Commit "时间" for "shijian" to learn it
        type("shijian")
        space()  // commits 时间 (from system dict)

        // Type again — 时间 should still be first (system dict), not duplicated
        let state = type("shijian")
        XCTAssertEqual(state.candidates.first, "时间")
        let count = state.candidates.filter { $0 == "时间" }.count
        XCTAssertEqual(count, 1, "Existing system dict phrase should not be duplicated")
    }
}
