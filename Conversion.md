...
**词 (word)**：在本算法语境下严格定义为 `word.count ≥ 2` 且 `freq ≥ 10000` 的词库条目。

- `word.count < 2`（单字）→ 不是词
- `word.count ≥ 2` 但 `freq < 10000` → 不是词（视为词库噪声，例如「的脚」freq=5555）

freq ≥ 10000 阈值是算法内部约定，用来过滤低质量词库条目，只承认"真正成词"的那部分。
...
**含义**：路径中词段的数量。
**形式化**：`wordCount = |{ s ∈ path.segments : s.word.count ≥ 2 ∧ s.freq ≥ 10000 }|`
